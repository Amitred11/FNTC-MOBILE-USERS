...
export const SubscriptionProvider = ({ children }) => {
    ...
    const refreshSubscription = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            // This API call now returns a fully populated object from the fixed backend.
    ...
    const subscribeToPlan = async (plan, paymentMethod, proofOfPayment, installationAddress) => {
        const payload = { plan, paymentMethod, installationAddress, proofOfPayment: proofOfPayment || null };
    ...
    const payBill = async (billId, proofOfPayment) => {
        const { data } = await api.post('/billing/pay', { billId, proofOfPayment: proofOfPayment || null });
    ...
    const value = {
        ...
        dataUsage: subscriptionData?.dataUsage || null, // Assuming backend might send this
